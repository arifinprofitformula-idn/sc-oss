    ...
    <script>
        function storeSettings() {
            ...
            return {
                ...
                banks: (initialData.bank_details || []).map(b => ({ ...b, _id: Math.random().toString(36).substr(2, 9) })),
                ...
                addBank() {
                    this.banks.push({ bank: '', number: '', name: '', _id: Math.random().toString(36).substr(2, 9) });
                },
    ...
    <div class="py-12" x-data="storeSettings()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('admin.settings.store.update') }}" method="POST" enctype="multipart/form-data" x-ref="mainForm">
                ...
                <!-- Store Details Tabs -->
                <div class="mx-[10px] sm:mx-0 bg-white dark:bg-gray-800 overflow-visible shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="mb-6">
                            @php
                                ...
                                $activeItem = collect($navTabs)->firstWhere('id', $currentTab);
                            @endphp
                                ...
                                <div class="relative">
                                    ...
                                    >
                                        ...
                                        <span class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                                            <svg class="h-5 w-5 text-gray-400 transition-transform duration-300" :class="{'rotate-180': open}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                    ...
                                    >
                                        @foreach($navTabs as $t)
                                            <li 
                                                ...
                                                role="option"
                                                @click="window.location.href = '{{ route('admin.settings.store.tab', ['tab' => $t['id']]) }}'"
                                            >
                                                <div class="flex items-center">
                                                    <span class="font-normal block truncate {{ $currentTab === $t['id'] ? 'font-semibold text-indigo-600 dark:text-indigo-400' : '' }}">
                                                        {{ $t['label'] }}
                                                    </span>
                                                </div>
                                ...
                                <nav class="-mb-px flex space-x-8 overflow-x-auto" aria-label="Tabs" role="tablist">
                                    @foreach($navTabs as $t)
                                        <a href="{{ route('admin.settings.store.tab', ['tab' => $t['id']]) }}"
                                           class="whitespace-nowrap pb-4 px-3 md:px-4 border-b-2 font-medium text-sm md:text-base focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded transition-colors duration-150 ease-in-out {{ $currentTab === $t['id'] 
                        ...
                        <div x-show="activeTab === 'payment'">
                            ...
                            <div class="space-y-4">
                                ...
                                <template x-for="(bank, index) in banks" :key="bank._id">
