        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mx-[10px] sm:mx-0 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.packages.store') }}" x-data="packageForm()" @submit.prevent="submitForm" enctype="multipart/form-data">
                        @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="commission_type" id="commission_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        <option value="fixed" {{ old('commission_type', 'fixed') == 'fixed' ? 'selected' : '' }}>Nilai Tetap (IDR)</option>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="commission_value" id="commission_value" min="0" step="0.01" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required value="{{ old('commission_value', 0) }}">
                        ...
                        <div class="mb-6">
                            <div class="flex items-center">
                                ...
                                <input type="checkbox" name="is_active" id="is_active" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" {{ old('is_active', true) ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center justify-end">
                            ...
                            <button type="submit" :disabled="isLoading" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline flex items-center disabled:opacity-50 disabled:cursor-not-allowed">
                                ...
                                <span x-text="isLoading ? 'Menyimpan...' : 'Simpan Paket'"></span>
                            </button>
                        </div>
                    </form>
                </div>
                ...
                submitForm(event) {
                    ...
                    .then(async response => {
                        ...
                        if (contentType && contentType.includes('application/json')) {
                            ...
                            this.successMessage = data.message || 'Paket berhasil dibuat.';
                            window.scrollTo(0, 0);
                        } else {
                    ...
                    .catch(error => {
                        ...
                        if (error.errors) {
                             ...
                             this.errorMessage = error.message || 'Gagal menyimpan paket.';
                        }
                        window.scrollTo(0, 0);
                    });
                }
